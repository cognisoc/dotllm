...
using Dotllm.Vulkan.Native;

namespace Dotllm.Vulkan;

public sealed class VulkanBackend : IComputeBackend
{
    private readonly VulkanInterop? _vulkan;
    private readonly Dictionary<string, nint> _pipelines = [];
    private readonly Dictionary<string, (nint buffer, nint memory)> _persistentBuffers = [];
    private readonly List<(nint buffer, nint memory)> _scratchBuffers = [];
    ...
    public VulkanBackend(VulkanBackendOptions? options = null)
    {
        try
        {
            _vulkan = new VulkanInterop(options);
            _gpuAvailable = _vulkan.IsInitialized;
        }
        catch
        {
            _gpuAvailable = false;
    ...
    public void Dispose()
    {
        ...
        if (_vulkan != null)
        {
            foreach (var (buffer, memory) in _scratchBuffers)
            {
                _vulkan.DestroyBuffer(buffer);
                _vulkan.FreeMemory(memory);
            }
            _scratchBuffers.Clear();

            foreach (var (_, (buffer, memory)) in _persistentBuffers)
            {
                _vulkan.DestroyBuffer(buffer);
                _vulkan.FreeMemory(memory);
            }
            ...
            foreach (var (_, pipeline) in _pipelines)
                _vulkan.DestroyPipeline(pipeline);
            _pipelines.Clear();
