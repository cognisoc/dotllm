...
public sealed class LlmdotChatClient : IChatClient
{
    ...
    private readonly LlmdotOptions _options;
    private readonly ChatClientMetadata _metadata;

    public LlmdotChatClient(IOptions<LlmdotOptions> options)
    {
        _options = options.Value;

        if (string.IsNullOrWhiteSpace(_options.ModelPath))
            throw new InvalidOperationException("LlmdotOptions.ModelPath must be set.");
    ...
    public object? GetService(Type serviceType, object? serviceKey = null) =>
        serviceType == typeof(LlmdotChatClient) ? this : null;
